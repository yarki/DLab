...
full_path <- function(file_path) {
    working_bucket <- "dlab6th-dmytro-liaskovskyi-bucket"
    sprintf("s3a://%s/%s", working_bucket, file_path)
}
#######################################################
carriers <- read.df(full_path("R_processed_emr/carriers"), "parquet")
createOrReplaceTempView(carriers, "carriers")
printSchema(carriers)
#######################################################
airports <- read.df(full_path("R_processed_emr/airports"), "parquet")
createOrReplaceTempView(airports, "airports")
printSchema(airports)
#######################################################
flights <- read.df(full_path("R_processed_emr/flights"), "parquet")
