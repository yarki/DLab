...
public class JenkinsCall {
    ...
    public String getBuildNumber() throws Exception {
        ...
        String builName = given().header("Authorization", "Basic YWRtaW46Vmxlc3VSYWRpbGFzRWxrYQ==").auth().form(Credential.JENKINS_USERNANE, Credential.JENKINS_PASSWORD, config).
            contentType("application/x-www-form-urlencoded").
            when(). 
    ...
    public String getBuildResult() throws Exception {
        ...
        do{            
            buildResult = given().header("Authorization", "Basic YWRtaW46Vmxlc3VSYWRpbGFzRWxrYQ==").auth().form(Credential.JENKINS_USERNANE, Credential.JENKINS_PASSWORD, config).
    ...
    public void setJenkinsURLServiceBaseName() throws Exception {
        ...
        if(buildResult.equals("SUCCESS")){
            ...
            jenkinsHoleURL = given().header("Authorization", "Basic YWRtaW46Vmxlc3VSYWRpbGFzRWxrYQ==").auth().form(Credential.JENKINS_USERNANE, Credential.JENKINS_PASSWORD, config).
