...
def start_ss():
    try:
        if not exists('/opt/dlab/tmp/ss_started'):
            ...
            try:
                local('scp -r -i {} /root/web_app/self-service/* {}:'.format(args.keyfile,
                                                                             env.host_string) + web_path + 'self-service/')
                local('scp -r -i {} /root/web_app/security-service/* {}:'.format(args.keyfile,
                                                                                 env.host_string) + web_path + 'security-service/')
                local('scp -r -i {} /root/web_app/provisioning-service/* {}:'.format(args.keyfile,
                                                                                     env.host_string) + web_path + 'provisioning-service/')
            except:
