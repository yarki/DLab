...
def status():
    with settings(warn_only=True):
        ...
        try:
            ...
            if local("~/scripts/%s.py %s" % ('collect_data', params)).failed:
...
def run():
    with settings(warn_only=True):
        ...
        try:
            ...
            if local("~/scripts/%s.py %s" % ('create_policy', params)).failed:
                with open("/root/result.json", 'w') as result:
        ...
        try:
            ...
            if local("~/scripts/%s.py %s" % ('configure_http_proxy', params)).failed:
                with open("/root/result.json", 'w') as result:
        ...
        try:
            ...
            params = "--hostname {} --keyfile {} --additional_config '{}' --user {}".format(
                instance_hostname, keyfile_name, json.dumps(additional_config), os.environ['general_os_user'])
            if local("~/scripts/%s.py %s" % ('install_user_key', params)).failed:
                with open("/root/result.json", 'w') as result:
...
def terminate():
    with settings(warn_only=True):
        ...
        try:
            ...
            if local("~/scripts/%s.py %s" % ('terminate_edge', params)).failed:
                with open("/root/result.json", 'w') as result:
