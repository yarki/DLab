...
def run():
    ...
    #     'edge_user_name'] + '-nb-' + str(provide_index('EC2', '{}-Tag'.format(os.environ['conf_service_base_name']), '{}-{}-nb'.format(os.environ['conf_service_base_name'], os.environ['edge_user_name'])))
    notebook_config['instance_name'] = os.environ['conf_service_base_name'] + "-" + os.environ['edge_user_name'] + "-nb-" + notebook_config['exploratory_name'] + "-" + notebook_config['uuid']
    notebook_config['expected_ami_name'] = os.environ['conf_service_base_name'] + "-" + os.environ[
        'edge_user_name'] + '-jupyter-notebook-image'
...
def stop():
    ...
    try:
        ...
        params = "--bucket_name {} --tag_name {} --nb_tag_value {} --ssh_user {} --key_path {} --edge_user_name {}"\
            .format(notebook_config['bucket_name'], notebook_config['tag_name'], notebook_config['notebook_name'], os.environ['conf_os_user'], notebook_config['key_path'], os.environ['edge_user_name'])
        try:
